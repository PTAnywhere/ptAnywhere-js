...
ptAnywhere.websocket = (function() {
  ...
  */
  var history = (function() {
    ...
    function getNextCommand() {
      ...
      if (currentIndex === commands.length - 1)
        currentIndex++; // current can still be moved to last position
  ...
  function connect(target) {
    ...
    if ('WebSocket' in window) {
      ...
      var msg = 'WebSocket is not supported by this browser.';
      console.error(msg);
  ...
  */
  function init(websocketURL, connectedCallback, outputCallback,
                    replaceCommandCallback, warningCallback) {
  ...
  */
  function send(msg) {
    ws.send(msg);
    // To ensure that an updated version is get the next time...
